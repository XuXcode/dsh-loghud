...
function healthColor(health: SessionSnapshot['health']): string { return health === 'BROKEN' ? '#ef4444' : health === 'HEALTHY' ? '#22c55e' : '#94a3b8' }
...
const styles: Record<string, import('react').CSSProperties> = {
  root: { pointerEvents: 'auto', position: 'fixed', right: 16, top: 12, zIndex: 50, font: '13px/1.45 system-ui, sans-serif', color: 'var(--color-fg, var(--foreground, #e5e7eb))' },
  dragging: { userSelect: 'none' },
  badge: { border: '1px solid', borderRadius: 999, padding: '7px 11px', background: 'var(--color-bg-elevated, var(--background, #171717))', color: 'inherit', cursor: 'pointer', boxShadow: '0 4px 18px #0003' },
  dot: { display: 'inline-block', width: 8, height: 8, borderRadius: 99, marginRight: 5 }, panel: { position: 'absolute', right: 0, top: 42, width: 'min(390px, calc(100vw - 24px))', maxHeight: 'calc(100vh - 66px)', overflow: 'auto', border: '1px solid var(--color-border, #404040)', borderRadius: 12, background: 'var(--color-bg, var(--background, #111))', boxShadow: '0 18px 60px #0008' },
  header: { display: 'flex', justifyContent: 'space-between', alignItems: 'start', padding: 14, borderBottom: '1px solid var(--color-border, #333)', touchAction: 'none' }, grip: { display: 'inline-block', marginRight: 6, color: 'var(--color-fg-muted, #888)' }, health: { marginTop: 3, color: 'var(--color-fg-muted, #a3a3a3)' }, icon: { border: 0, background: 'transparent', color: 'inherit', fontSize: 24, cursor: 'pointer' }, mode: { margin: 12, padding: 10, borderRadius: 8, background: 'var(--color-bg-subtle, #222)', color: 'var(--color-fg-muted, #aaa)' }, alert: { margin: 12, padding: 10, borderRadius: 7, background: '#7f1d1d55', color: '#fecaca' },
  section: { padding: '4px 12px 8px' }, h2: { fontSize: 12, textTransform: 'uppercase', letterSpacing: '.05em', margin: '10px 0 7px', color: 'var(--color-fg-muted, #a3a3a3)' }, count: { fontWeight: 400 }, card: { display: 'flex', width: '100%', flexDirection: 'column', gap: 4, textAlign: 'left', padding: 10, marginBottom: 7, border: '1px solid var(--color-border, #333)', borderRadius: 8, background: 'var(--color-bg-elevated, #191919)', color: 'inherit', cursor: 'pointer' }, cardSelected: { borderColor: '#60a5fa', boxShadow: '0 0 0 1px #60a5fa' }, summary: { color: 'var(--color-fg-muted, #bbb)' }, empty: { color: 'var(--color-fg-muted, #999)', padding: '4px 12px' },
  details: { margin: 12, padding: 10, border: '1px solid var(--color-border, #333)', borderRadius: 8 }, dl: { display: 'grid', gridTemplateColumns: '90px 1fr', gap: 5, overflowWrap: 'anywhere' }, pre: { maxHeight: 220, overflow: 'auto', whiteSpace: 'pre-wrap', padding: 8, borderRadius: 6, background: '#0005', fontSize: 11 }, diagnosis: { marginTop: 10, padding: 10, background: '#14532d44', borderRadius: 7 }, diagnosisHeading: { margin: '12px 0 4px', fontSize: 12 }, notice: { marginTop: 10, padding: 10, borderRadius: 7, background: '#78350f55', color: '#fde68a' }, actions: { display: 'flex', flexWrap: 'wrap', gap: 7, marginTop: 10 }, primary: { border: 0, borderRadius: 7, padding: '7px 10px', background: '#2563eb', color: 'white', cursor: 'pointer' }, secondary: { border: '1px solid var(--color-border, #444)', borderRadius: 7, padding: '6px 9px', background: 'transparent', color: 'inherit', cursor: 'pointer' }, danger: { border: '1px solid #b91c1c', borderRadius: 7, padding: '6px 9px', background: 'transparent', color: '#fca5a5', cursor: 'pointer' }, footer: { display: 'flex', gap: 8, justifyContent: 'flex-end', padding: 12, borderTop: '1px solid var(--color-border, #333)' },
  settings: { margin: 12, padding: 10, border: '1px solid var(--color-border, #333)', borderRadius: 8 },
}
